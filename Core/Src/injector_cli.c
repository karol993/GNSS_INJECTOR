...
static void process(char *command) {
  InjectorUtc time; int32_t latitude_udeg, longitude_udeg; long offset; char arg[8]; char status[320]; size_t i;
  for (i = 0U; command[i] != '\0'; ++i) command[i] = (char)toupper((unsigned char)command[i]);
  InjectorStats_CliCommand();
  if (strcmp(command,"HELP") == 0) reply("INJ,HELP,START,STOP,STATUS,STATS,TXTEST,MODE,TIME,POS,FIX,PPS,RMC,ZDA,OFFSET,RESET\r\n");
  ...
  else if (strcmp(command,"MODE") == 0) { reply(INJECTOR_GetMode() == INJECTOR_MODE_LC29H ? "INJ,MODE,LC29H\r\n" : "INJ,MODE,MINIMAL\r\n"); }
  else if (strcmp(command,"MODE MINIMAL") == 0) { INJECTOR_SetMode(INJECTOR_MODE_MINIMAL); reply("INJ,OK\r\n"); }
  else if (strcmp(command,"MODE LC29H") == 0) { INJECTOR_SetMode(INJECTOR_MODE_LC29H); reply("INJ,OK\r\n"); }
