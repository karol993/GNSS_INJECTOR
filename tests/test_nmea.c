...
int main(void) {
  ...
  utc = (InjectorUtc){2026U,9U,15U,10U,0U,0U}; assert(NMEA_FormatRMC(text,sizeof text,&utc,50.0f,19.0f,'A') > 0); assert(strcmp(text,"$GNRMC,100000.00,A,5000.0000,N,01900.0000,E,0.0,0.0,150926,,,A*45\r\n") == 0);
  assert(NMEA_FormatRMC(text,sizeof text,&utc,-50.0f,-19.0f,'V') > 0); assert(strstr(text,",5000.0000,S,01900.0000,W,") != 0);
